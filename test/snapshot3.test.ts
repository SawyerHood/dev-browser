...
import { withPage, closeBrowser, type DoobiePage } from "./helpers/browser.ts";
...
async function framesReady(page: DoobiePage, n: number) {
...
test("reinstalling the in-page script never reuses ref ids already handed out", async () => {
  await withPage(async (page) => {
    ...
    await realm.evaluate(() => {
      delete (window as unknown as { __doobie?: unknown }).__doobie;
    });
    ...
    const v = await realm.evaluate(() => (window as unknown as { __doobie: { version: number } }).__doobie.version);
