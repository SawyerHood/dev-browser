/**
 * `dev-browser mcp`: stdio JSON-RPC server over the same frame contract.
 */
...
beforeAll(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "dev-browser-mcp-"));
  const env = { ...process.env, DEV_BROWSER_HOME: home } as Record<string, string>;
...
afterAll(async () => {
  try {
    await rpc("tools/call", { name: "dev_browser_stop", arguments: {} });
  } catch {
    /* ignore */
  }
  proc.kill();
...
test("initialize + tools/list", async () => {
  ...
  expect(init.result.serverInfo.name).toBe("dev-browser");
  ...
  expect(names).toEqual(["dev_browser_run", "dev_browser_pages", "dev_browser_browsers", "dev_browser_stop", "dev_browser_help"]);
}, 20_000);

test("dev_browser_run returns text + image content, errors set isError", async () => {
  ...
  const e = await rpc("tools/call", { name: "dev_browser_run", arguments: { script: "throw new TypeError('boom')" } });
  expect(e.result.isError).toBe(true);
  expect(e.result.content[0].text).toContain("TypeError: boom");

  const pages = await rpc("tools/call", { name: "dev_browser_pages", arguments: {} });
  expect(pages.result.content[0].text).toContain("  m  ");
  const help = await rpc("tools/call", { name: "dev_browser_help", arguments: { topic: "refs" } });
