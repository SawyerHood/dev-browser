...
describe("client transport: Bun.connect backpressure", () => {
  test("a 2 MB payload arrives intact when the server reads slowly", async () => {
    const dir = tmpDir("dev-browser-conn-");
...
describe("profile lock safety predicate (classifyLockHolder)", () => {
  const dir = "/home/u/.dev-browser/browsers/work/profile";
  ...
  test("chromePortPids reads pids recorded by `dev-browser chrome`", () => {
    const home = tmpDir("dev-browser-home-");
    const prev = process.env.DEV_BROWSER_HOME;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.DEV_BROWSER_HOME;
      else process.env.DEV_BROWSER_HOME = prev;
    }
  });
});

describe("profile dirs", () => {
  test("headed and headless instances of one name use different user-data-dirs; dev-browser chrome has its own root", () => {
    const home = tmpDir("dev-browser-home-");
    const prev = process.env.DEV_BROWSER_HOME;
    process.env.DEV_BROWSER_HOME = home;
    try {
...
describe("Chrome for Testing discovery (installedCandidates)", () => {
  ...
  test("finds the linux and the (deep) macOS layouts under a temp DEV_BROWSER_HOME", () => {
    const home = tmpDir("dev-browser-home-");
    const prev = process.env.DEV_BROWSER_HOME;
    process.env.DEV_BROWSER_HOME = home;
    try {
...
describe("dev-browser chrome picks the user's real browser", () => {
...
describe("read-only DEV_BROWSER_HOME", () => {
  test("ensureHome fails fast with a clear message", () => {
    ...
    const base = tmpDir("dev-browser-ro-");
    const home = path.join(base, "home");
    fs.mkdirSync(home, { mode: 0o500 });
    const prev = process.env.DEV_BROWSER_HOME;
    process.env.DEV_BROWSER_HOME = home;
    try {
      expect(() => ensureHome()).toThrow(/cannot write to DEV_BROWSER_HOME .*EACCES/);
    } finally {
