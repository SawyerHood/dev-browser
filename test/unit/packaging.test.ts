 ...
 * relinkGlobal gating), the bin/dev-browser.cjs shim's self-heal path, postinstall
...
const NO_BUN_FALLBACK = /fall ?back to bun|TypeScript entry|runs? .*with bun|DEV_BROWSER_SKIP_DOWNLOAD=1 npm rebuild/i;
const FAKE_BIN = Buffer.from(`#!/bin/sh\necho "fake-dev-browser $*"\nexit 0\n`);
const tmpDirs: string[] = [];
function tmp(prefix = "dev-browser-pkg-"): string {
...
beforeAll(async () => {
  srv = await startServer({
    ...
    "/good/SHA256SUMS": () => `${sha256("other")}  dev-browser-other-thing\n${sha256(FAKE_BIN)}  ${ASSET}\n`,
    ...
    "/nosum/SHA256SUMS": () => `${sha256(FAKE_BIN)}  dev-browser-plan9-mips\n`,
...
describe("download-binary module", () => {
  test("platformAsset names a supported asset on this CI platform", () => {
    expect(ASSET).toMatch(/^dev-browser-(linux|darwin)-(x64|arm64)$/);
  });
  ...
  test("DEV_BROWSER_DOWNLOAD_BASE overrides the base (trailing slash stripped)", () => {
    const prev = process.env.DEV_BROWSER_DOWNLOAD_BASE;
    try {
      process.env.DEV_BROWSER_DOWNLOAD_BASE = "http://mirror.example/dev-browser/v1/";
      expect(dl.downloadBase()).toBe("http://mirror.example/dev-browser/v1");
      delete process.env.DEV_BROWSER_DOWNLOAD_BASE;
      expect(dl.downloadBase()).toBe(dl.defaultBase());
    } finally {
      if (prev === undefined) delete process.env.DEV_BROWSER_DOWNLOAD_BASE;
      else process.env.DEV_BROWSER_DOWNLOAD_BASE = prev;
    }
  });
  ...
  test("downloads, verifies and installs the binary (mode 755, atomic)", async () => {
    const dest = path.join(tmp(), "bin", "dev-browser-bin");
    const logs: string[] = [];
    ...
    expect(fs.readdirSync(path.dirname(dest))).toEqual(["dev-browser-bin"]); // no leftover .tmp
    expect(logs.join("\n")).toContain(`downloading ${ASSET} v${dl.VERSION} from ${goodBase}`);
  });

  test("follows redirects", async () => {
    const dest = path.join(tmp(), "dev-browser-bin");
  ...
  test("refuses a checksum mismatch and leaves nothing behind", async () => {
    const dir = tmp();
    const dest = path.join(dir, "dev-browser-bin");
  ...
  test("DEV_BROWSER_DOWNLOAD_BASE env drives downloadBinary when no base is given", async () => {
    const prev = process.env.DEV_BROWSER_DOWNLOAD_BASE;
    const dest = path.join(tmp(), "dev-browser-bin");
    try {
      process.env.DEV_BROWSER_DOWNLOAD_BASE = goodBase + "/";
...
describe("relinkGlobal", () => {
  /** prefix/bin/dev-browser -> prefix/lib/node_modules/dev-browser/bin/dev-browser.cjs (npm's global layout). */
  function fakeGlobal() {
    const prefix = tmp("dev-browser-prefix-");
    const pkgBin = path.join(prefix, "lib", "node_modules", "dev-browser", "bin");
    fs.mkdirSync(pkgBin, { recursive: true });
    fs.mkdirSync(path.join(prefix, "bin"));
    const shim = path.join(pkgBin, "dev-browser.cjs");
    fs.writeFileSync(shim, "// shim\n");
    const dest = path.join(pkgBin, "dev-browser-bin");
  ...
  test("does not hijack a global link that points at a different dev-browser package", () => {
    const g = fakeGlobal();
    // "this" package lives elsewhere (e.g. a project's node_modules) but the env says global.
    const other = tmp("dev-browser-other-");
    fs.mkdirSync(path.join(other, "bin"));
    const otherShim = path.join(other, "bin", "dev-browser.cjs");
    fs.writeFileSync(otherShim, "// other shim\n");
    const otherDest = path.join(other, "bin", "dev-browser-bin");
...
function fakePackage(): string {
  const dir = tmp("dev-browser-npmpkg-");
  fs.mkdirSync(path.join(dir, "bin"));
  fs.mkdirSync(path.join(dir, "scripts"));
  fs.copyFileSync(path.join(ROOT, "bin/dev-browser.cjs"), path.join(dir, "bin/dev-browser.cjs"));
...
describe("bin/dev-browser.cjs shim", () => {
  test("self-heals: downloads the binary on first run, then runs it; second run does not download", async () => {
    const pkg = fakePackage();
    const shim = path.join(pkg, "bin/dev-browser.cjs");
    const hitsBefore = srv.hits.length;
    const r1 = await runNode(shim, ["--version", "--x"], { DEV_BROWSER_DOWNLOAD_BASE: goodBase }, pkg);
    expect(r1.stderr).toContain(`dev-browser: downloading binary v${dl.VERSION}...`);
    expect(r1.stdout.trim()).toBe("fake-dev-browser --version --x");
    expect(r1.code).toBe(0);
    expect(fs.existsSync(path.join(pkg, "bin/dev-browser-bin"))).toBe(true);
    ...
    const r2 = await runNode(shim, ["pages"], { DEV_BROWSER_DOWNLOAD_BASE: goodBase }, pkg);
    expect(r2.stderr).toBe("");
    expect(r2.stdout.trim()).toBe("fake-dev-browser pages");
    expect(srv.hits.length).toBe(hitsMid);
  });

  test("passes the binary's exit code through", async () => {
    const pkg = fakePackage();
    fs.writeFileSync(path.join(pkg, "bin/dev-browser-bin"), "#!/bin/sh\necho err >&2\nexit 124\n", { mode: 0o755 });
    const r = await runNode(path.join(pkg, "bin/dev-browser.cjs"), [], {}, pkg);
  ...
  test("offline / unreachable mirror: clear error, exit 1, no bun advice", async () => {
    const pkg = fakePackage();
    const r = await runNode(path.join(pkg, "bin/dev-browser.cjs"), ["--version"], { DEV_BROWSER_DOWNLOAD_BASE: "http://127.0.0.1:1/x" }, pkg);
    expect(r.code).toBe(1);
    expect(r.stderr).toContain("dev-browser: downloading binary");
    expect(r.stderr).toContain("could not download the binary");
    expect(r.stderr).toContain("npm rebuild -g dev-browser");
    expect(r.stderr).toContain("http://127.0.0.1:1/x");
    expect(r.stderr).not.toMatch(NO_BUN_FALLBACK);
    expect(fs.existsSync(path.join(pkg, "bin/dev-browser-bin"))).toBe(false);
  });

  test("checksum mismatch refuses the binary", async () => {
    const pkg = fakePackage();
    const r = await runNode(path.join(pkg, "bin/dev-browser.cjs"), ["--version"], { DEV_BROWSER_DOWNLOAD_BASE: badSumBase }, pkg);
    expect(r.code).toBe(1);
    expect(r.stderr).toContain("checksum mismatch");
    expect(fs.existsSync(path.join(pkg, "bin/dev-browser-bin"))).toBe(false);
  });

  test("DEV_BROWSER_SKIP_DOWNLOAD: does not download, explains how to fix", async () => {
    const pkg = fakePackage();
    const hits = srv.hits.length;
    const r = await runNode(path.join(pkg, "bin/dev-browser.cjs"), ["--version"], { DEV_BROWSER_DOWNLOAD_BASE: goodBase, DEV_BROWSER_SKIP_DOWNLOAD: "1" }, pkg);
    expect(r.code).toBe(1);
    expect(r.stderr).toContain("DEV_BROWSER_SKIP_DOWNLOAD is set");
    expect(r.stderr).toContain("npm rebuild -g dev-browser");
    expect(r.stderr).not.toMatch(NO_BUN_FALLBACK);
  ...
  test("dev checkout: the real shim in this repo runs src/cli/main.ts with bun when no binary exists", async () => {
    // bin/dev-browser-bin is never present in the checkout; the shim must mention bun only here.
    ...
    const r = await runNode(path.join(ROOT, "bin/dev-browser.cjs"), ["--version"], { PATH: process.env.PATH! }, ROOT);
    expect(r.code).toBe(0);
    expect(r.stdout.trim()).toBe(`dev-browser ${dl.VERSION}`);
  });
});

describe("scripts/postinstall.cjs", () => {
  test("installs from DEV_BROWSER_DOWNLOAD_BASE and does not touch a global link for a local install", async () => {
    const pkg = fakePackage();
    const prefix = tmp("dev-browser-prefix-");
    fs.mkdirSync(path.join(prefix, "bin"));
    const link = path.join(prefix, "bin", "dev-browser");
    fs.symlinkSync(path.join(pkg, "bin/dev-browser.cjs"), link);
    const r = await runNode(path.join(pkg, "scripts/postinstall.cjs"), [], { DEV_BROWSER_DOWNLOAD_BASE: goodBase, npm_config_prefix: prefix }, pkg);
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("installed binary at");
    expect(fs.readFileSync(path.join(pkg, "bin/dev-browser-bin"))).toEqual(FAKE_BIN);
    expect(fs.readlinkSync(link)).toBe(path.join(pkg, "bin/dev-browser.cjs")); // untouched
    expect(r.stdout).not.toContain("points directly");
  });

  test("global install relinks the global `dev-browser` symlink at the binary", async () => {
    const pkg = fakePackage();
    const prefix = tmp("dev-browser-prefix-");
    fs.mkdirSync(path.join(prefix, "bin"));
    const link = path.join(prefix, "bin", "dev-browser");
    ...
    const r = await runNode(
      ...
      [],
      { DEV_BROWSER_DOWNLOAD_BASE: goodBase, npm_config_prefix: prefix, npm_config_global: "true" },
      pkg,
    );
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("points directly at the binary");
    expect(fs.realpathSync(fs.readlinkSync(link))).toBe(fs.realpathSync(path.join(pkg, "bin/dev-browser-bin")));
  });

  test("download failure: exit 0, message recommends npm rebuild / manual download, never bun", async () => {
    const pkg = fakePackage();
    const r = await runNode(path.join(pkg, "scripts/postinstall.cjs"), [], { DEV_BROWSER_DOWNLOAD_BASE: srv.url("/missing") }, pkg);
    expect(r.code).toBe(0);
    ...
    expect(r.stderr).toContain("bin/dev-browser-bin");
    expect(r.stderr).not.toMatch(NO_BUN_FALLBACK);
  });

  test("DEV_BROWSER_SKIP_DOWNLOAD skips; dev checkout (src/ present) is a no-op", async () => {
    const pkg = fakePackage();
    const r = await runNode(path.join(pkg, "scripts/postinstall.cjs"), [], { DEV_BROWSER_SKIP_DOWNLOAD: "1", DEV_BROWSER_DOWNLOAD_BASE: goodBase }, pkg);
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("DEV_BROWSER_SKIP_DOWNLOAD set");
    expect(fs.existsSync(path.join(pkg, "bin/dev-browser-bin"))).toBe(false);
    const r2 = await runNode(path.join(ROOT, "scripts/postinstall.cjs"), [], { DEV_BROWSER_DOWNLOAD_BASE: "http://127.0.0.1:1/x" }, ROOT);
