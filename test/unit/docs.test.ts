/**
 * Docs consistency: docs/help.md (embedded in `dev-browser --help`) must stay within
...
const SKILL = fs.readFileSync(path.join(ROOT, "skills/dev-browser/SKILL.md"), "utf8");
const DESIGN = fs.readFileSync(path.join(ROOT, "docs/design-decisions.md"), "utf8");

const cli = makeCliEnv("dev-browser-docs-");
afterAll(async () => {
  await cli.cleanup();
});
...
test("help.md stays within the line budget and every topic is present", () => {
  ...
  expect(HELP).toContain("dev-browser chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [--list] [URL]");
...
test("README, SKILL and design doc reflect the current behaviour", () => {
  expect(README).toContain("medians of 9 runs");
  expect(README).not.toContain("~570 ms");
  expect(README).toContain("DEV_BROWSER_DOWNLOAD_BASE");
  expect(README).toContain("## Releasing");
  expect(README).toContain("bring-to-front lock");
  expect(SKILL).toContain("bun add -g dev-browser");
  expect(README).toContain("migrate-from-doobie");
