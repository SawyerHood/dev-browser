/**
 * `dev-browser chrome` launch verification: a Chrome that dies right away is an
...
async function chrome(args: string[], env: Record<string, string> = {}): Promise<{ code: number; stdout: string; stderr: string }> {
  ...
  const proc = Bun.spawn([process.execPath, runner, ...args], {
    cwd: ROOT,
    env: { ...process.env, DEV_BROWSER_HOME: home, NODE_PATH: "", ...env },
    stdout: "pipe",
    stderr: "pipe",
  });
...
beforeAll(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "dev-browser-chrome-cmd-"));
  prevHome = process.env.DEV_BROWSER_HOME;
  process.env.DEV_BROWSER_HOME = home;
  cli = makeCliEnv("dev-browser-chrome-cli-");
});
afterAll(async () => {
...
describe("dev-browser chrome verifies the launch", () => {
...
describe("dev-browser help <unknown topic>", () => {
  test("exits 2 with the message on stderr; known topics still exit 0", async () => {
    const r = await cli.run(["help", "bogus-topic"]);
    expect(r.code).toBe(2);
    expect(r.stdout).toBe("");
    expect(r.stderr).toMatch(/^dev-browser: No help topic "bogus-topic"\. Topics: .*quickstart/);
