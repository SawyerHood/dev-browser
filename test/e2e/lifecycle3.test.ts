 ...
 * tracker) while dev-browser's pages, their popups, and tabs reached via
...
beforeAll(async () => {
  cli = makeCliEnv("dev-browser-e2e-life3-");
...
/**
 * From the user's own client: run confirm() in a tab. If dev-browser's dialog
...
describe("--connect: the user's own tabs are not extended", () => {
  ...
  beforeAll(async () => {
    ...
    port = await freePort();
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dev-browser-ext3-chrome-"));
  ...
  test("a tab the user opens while dev-browser is attached keeps its dialogs and gets no load tracker", async () => {
    ...
    expect(await userPage.evaluate(() => typeof (window as unknown as { __devBrowserLoad?: unknown }).__devBrowserLoad)).toBe("undefined");
    // and dev-browser did not report it as one of its pages in that run's stderr (nothing ran)
  ...
  test("dev-browser's own named page auto-dismisses dialogs (extended)", async () => {
  ...
  test("a popup opened from a dev-browser page is extended (opener is a touched page)", async () => {
...
r`,
  ...
  test("a user tab reached via getPage(targetId) is extended lazily, on first access", async () => {
    ...
    // and it stays extended (dev-browser touched it): the user's confirm is now auto-dismissed
...
describe("pages/status do not reset --idle-timeout", () => {
  ...
  test("`dev-browser pages -b NAME` and `status` keep the previously chosen idle timeout; a run with a new one changes it", async () => {
  ...
  test("`dev-browser pages -b NEW` on a not-yet-running browser launches it with the default idle timeout", async () => {
