 ...
 *
 * Names live in ~/.dev-browser/v1/pages/<browserKey>.json as { name: targetId } so
...
import { extendPage, type DevBrowserPage } from "../page/extend.ts";
...
export class PageRegistry {
  ...
  /**
   * Target ids of pages dev-browser has extended (named pages, getPage(targetId),
   * newPage, popups of touched pages). On attached browsers (--connect) only
   * these carry dev-browser's dialog/load hooks; the user's own tabs are left alone.
   */
  ...
  /** Extend a page with the dev-browser helpers and record it as touched. */
  adopt(page: Page): DevBrowserPage {
    this.touched.add(targetIdOf(page));
    return extendPage(page);
  }

  /** True when dev-browser has extended the page with this target id. */
  ...
  /** Name an existing page (used by tests and future `dev-browser pages name` command). */
