...
export class BrowserManager {
  ...
  /** Pids of Chrome processes this daemon owns or knows to be the user's (`doobie chrome`). Never orphan candidates. */
        ...
        if (outcome !== "free") {
          throw new ProfileBusyError(
            `profile "${spec.name}" (${spec.headless ? "headless" : "headed"}) is in use by another Chrome (${outcome}). ` +
              `Stop it first (\`doobie stop ${spec.name}\`, or quit that Chrome), or use a different -b NAME.`,
          );
        }
        r = await launchBrowser(spec, this.log, { timeoutMs: opts.timeoutMs });
      }
 ...
 * (after `doobie stop`, a crash or a SIGKILLed daemon). Those tabs are
...
/**
 * Launched browsers download into ~/.doobie/tmp/downloads instead of the
...
/** Pids recorded by `doobie chrome` (the user's signed-in Chromes). */
...
export type LockHolder =
  ...
  /** Live Chrome owned by this daemon or by `doobie chrome`: never touch. */
 ...
 * pids that must never be killed (live BrowserManager entries, `doobie
      ...
      case "unrelated":
        if (Date.now() >= deadline) {
          return holder.kind === "ours"
            ? `pid ${holder.pid}, owned by this daemon or \`doobie chrome\``
            : holder.kind === "foreign"
