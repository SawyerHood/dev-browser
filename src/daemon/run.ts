...
export async function runScript(req: RunRequest, ctx: RunContext): Promise<RunOutcome> {
  ...
  void (async () => {
    ...
    const context = vm.createContext(sandbox, { name: "dev-browser-script" });
...
async function buildErrorFrame(
  ...
  // `dev-browser stop` (or a crashed Chrome) under a running script: name the cause instead of Puppeteer's
