/**
 * `doobie mcp`: a Model Context Protocol server over stdio that exposes the
 * same request/frame contract the CLI uses. Hand-rolled JSON-RPC 2.0 (no SDK)
 * so the binary stays dependency-free.
 *
 * Tools:
 *   doobie_run      { script, browser?, headless?, connect?, timeout? }  -> text (+ image blocks for page.shot())
 *   doobie_pages    { browser?, connect? }
 *   doobie_browsers {}
 *   doobie_stop     { browser? }
 *   doobie_help     { topic? }
 *
 * Flags given to `doobie mcp` (-b, --headless, --connect, -t, --idle-timeout,
...
const TOOLS = [
  {
    name: "doobie_run",
    description:
      "Run a JavaScript snippet in doobie's browser runtime (Puppeteer). Globals: browser.getPage(name) (persistent named tabs), page.snapshot(), page.ref('e5') / 'ref/e5' selectors, page.shot(), page.waitForLoad(), page.fill(), saveFile/readFile. The last expression is the return value. Returns console output, the return value, errors, and page.shot() images.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  { name: "doobie_browsers", description: "List running browsers managed by the doobie daemon.", inputSchema: { type: "object", properties: {} } },
  {
    name: "doobie_stop",
  ...
  {
    name: "doobie_help",
    description: "The doobie usage guide (or one topic: quickstart, workflow, scripts, pages, snapshot, refs, screenshots, waiting, forms, errors, output, connect, chrome, config, json, examples, tips).",
      ...
      switch (msg.method) {
        case "initialize":
          reply(id, {
            ...
            capabilities: { tools: {} },
            serverInfo: { name: "doobie", version: VERSION },
            instructions:
              "Drive a browser with short Puppeteer scripts via doobie_run. Workflow: getPage(name) -> goto -> page.snapshot({interactive:true}) -> act via 'ref/eN' selectors -> verify with a tracked snapshot or page.shot(). Call doobie_help for the full guide.",
          });
          return;
