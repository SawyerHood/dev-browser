 ...
 * Installs window.__devBrowser (idempotent). See ./index.ts for the contract.
 *
...
export const INPAGE_SCRIPT: string = String.raw`(() => {
  ...
  // a newer dev-browser (higher INPAGE_VERSION) reinstalling into a long-lived page keeps handing out fresh ids
  // instead of restarting at e1 while old elements still carry their expando refs.
  const refState = window.__devBrowserRefState || (window.__devBrowserRefState = { lastRef: 0, refMap: new Map() });
  const refMap = refState.refMap; // "e5" -> WeakRef<Element> | Element
  const REF_KEY = "__devBrowserRef";
  ...
  window.__devBrowser = { version: ${INPAGE_VERSION}, snapshot, ref: refElement, box: refBox };
})()`;
