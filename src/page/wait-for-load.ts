 ...
 * tracker on window.__devBrowserLoad (PerformanceObserver('resource') for request
...
function pollInPage(staleMs: number): TrackerState {
  ...
  const w = window as unknown as { __devBrowserLoad?: Tracker };
  let t = w.__devBrowserLoad;
  if (!t) {
    t = { lastRequestStart: -1, lastMutation: -1, inFlight: new Map(), nextId: 1, installedAt: performance.now() };
    w.__devBrowserLoad = t;
    const tr = t;
