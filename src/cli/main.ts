/**
 * dev-browser CLI entry. Keep this module and its static imports small: the
...
import { DEFAULTS, loadConfigAsync, resolveIdleTimeoutMs, formatDuration, type DevBrowserConfig } from "../shared/config.ts";
...
export async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (err instanceof UsageError) {
      writeErr(`dev-browser: ${err.message}\n`);
      return EXIT_USAGE;
    }
    throw err;
  }
  ...
  switch (command.kind) {
    ...
    case "help": {
      ...
      if (text === null) {
        writeErr(`dev-browser: ${unknownTopicMessage(command.topic)}`);
        return EXIT_USAGE;
      }
      out(text);
      return EXIT_OK;
    ...
    case "migrate-from-doobie": {
      const mod = await loadDaemonModule();
      return mod.migrateFromDoobie();
    }
    case "chrome": {
      const mod = await loadDaemonModule();
...
export function sourceFromFlags(flags: GlobalFlags, config: DevBrowserConfig): BrowserSourceSpec {
...
async function runScriptCommand(flags: GlobalFlags, file?: string): Promise<number> {
  ...
  try {
    ...
    writeErr(`dev-browser: ${(err as Error).message}\n`);
    return EXIT_USAGE;
  }
...
export function runCli(): void {
  main(process.argv.slice(2)).then(
    ...
    (e) => {
      writeErr(`dev-browser: ${(e as Error)?.stack ?? String(e)}\n`);
      flushAll();
