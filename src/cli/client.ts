...
function acquireSpawnLock(): boolean {
  ...
  try {
    ...
    if (code !== "EEXIST") {
      throw new Error(
        `cannot write to DEV_BROWSER_HOME ${paths.home()} (${code ?? (err as Error).message}). Fix its permissions or point DEV_BROWSER_HOME at a writable directory.`,
      );
    }
    try {
...
export async function ensureDaemon(): Promise<Conn> {
  ...
  throw new Error(
    `dev-browser daemon did not start within ${SPAWN_WAIT_MS / 1000}s. Check ${paths.log()} and run \`dev-browser status\`.`,
  );
}
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (err instanceof DaemonHungError && attempt === 0) {
        writeErr("dev-browser: daemon unresponsive, restarting it\n");
        killDaemon();
        continue;
      }
      throw err;
