...
import { quickjsPlatform } from "../../quickjs-platform";
...
export class Connection extends EventEmitter {
  ...
  constructor(
    platform: Platform = quickjsPlatform,
  ...
  async sendMessageToServer(
    ...
    const location = options.frames?.[0]
      ...
      : undefined;
    const metadata: channels.Metadata = {
      title: options.title,
      location,
    ...
    if (this._tracingCount && options.frames && type !== "LocalUtils")
      this._localUtils
        ?.addStackToTracingNoReply({ callData: { stack: options.frames ?? [], id } })
        .catch(() => {});
  ...
  private _createRemoteObject(
    ...
    switch (type) {
      ...
      case "LocalUtils":
        ...
        if (!this._localUtils) this._localUtils = result as LocalUtils;
        break;
      case "Page":
