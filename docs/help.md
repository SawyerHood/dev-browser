...
USAGE
  dev-browser [flags] < script.js        run a script from stdin (heredoc)
  dev-browser [flags] -e 'code'          run inline code
  dev-browser [flags] run FILE           run a script file
  dev-browser pages | browsers | status | stop [NAME] | install [--force] | install-skill [--claude|--codex|--agents]
  dev-browser migrate-from-doobie       copy durable ~/.doobie state into the isolated v1 state directory
  dev-browser chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [--list] [URL]
  dev-browser help [topic]               topics: quickstart workflow scripts pages snapshot refs screenshots waiting forms
                                    errors output connect chrome config migration json mcp examples tips
...
## pages
  ...
  launched browsers land in ~/.dev-browser/v1/tmp/downloads/<name> (readFile("downloads/<name>") reads them); attached browsers
...
## screenshots
  ...
  Writes ~/.dev-browser/v1/tmp/shot-<ts>-<n>.jpg (JPEG q80) and prints `[image] PATH (WxH)`; open it with your image/Read tool.
...
## errors
  ...
  Chrome's error page). Recover by reconnecting to the same page name and checking state. `dev-browser stop` during a script
  ...
  "No Chrome found": `dev-browser install` or DEV_BROWSER_CHROME. Daemon trouble: `dev-browser status` (log tail), `dev-browser stop`, retry.
  "profile ... is in use by another Chrome (pid N)": a hand-started Chrome holds that profile dir; stop it or use another
  -b NAME. Snap Chromium (Ubuntu /snap/bin) cannot read ~/.dev-browser/v1: use `dev-browser install` or DEV_BROWSER_CHROME=<deb/CfT>.
...
## output
  ...
  Cap: after 50k chars of stdout the rest is spilled to ~/.dev-browser/v1/tmp/out-<id>.txt and at the end dev-browser prints
...
## connect
  --connect with no value = auto: probes ports remembered by `dev-browser chrome`, then 9222-9229 on 127.0.0.1 (400 ms each).
  ...
  Attached browsers are never closed by --idle-timeout; `dev-browser stop <key>` only disconnects. Named pages work the same
...
## chrome
  dev-browser chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [URL]   launch your real installed Chrome as
  a normal OS process with --remote-debugging-port on a dedicated profile (~/.dev-browser/v1/chrome-profiles/NAME, default NAME
  "chrome") and remember the port in ~/.dev-browser/v1/chrome-ports.json so a bare `dev-browser --connect` finds it. This is the path
  for Google sign-in and other logins that reject automation-launched Chrome: sign in by hand once, then automate.
  It prefers a system Chrome (then DEV_BROWSER_CHROME/config, then `dev-browser install`'s Chrome for Testing, then Playwright's
  ...
  (~/.dev-browser/v1/chrome-logs/NAME.log) are printed, exit 1, nothing recorded. On Linux a sandbox failure is retried once
  with --no-sandbox (remembered in launch-state.json). --headless passes --headless=new (CI/tests).
  `dev-browser chrome --list` prints detected Chromes. (Chrome 136+ ignores the port flag on its default profile, hence the
  dedicated --user-data-dir.) A --connect that finds nothing prints this launch hint.

## config
  ~/.dev-browser/v1/config.json: { "headless": false, "idleTimeout": "30m", "chrome": "/path/to/chrome", "timeout": 30, "ignoreHttpsErrors": false }
  Precedence everywhere: flag > env > config.json > default. Env: DEV_BROWSER_HOME (state dir, default ~/.dev-browser/v1:
       daemon.sock/.pid/.log, config.json, tmp/ (shots, out-*.txt, downloads/), browsers/NAME/profile[-headless]
       [-insecure], chrome-profiles/NAME, chrome-logs/, pages/, chrome/), DEV_BROWSER_CHROME (executable), DEV_BROWSER_IDLE_TIMEOUT
       (legacy DEV_BROWSER_IDLE_TIMEOUT_MS is also accepted),
       DEV_BROWSER_SOCKET. Durations: 30s, 5m, 1h, 0 (off), or milliseconds.
  Chrome lookup: DEV_BROWSER_CHROME > config.chrome > ~/.dev-browser/v1/chrome (from `dev-browser install`, Chrome for Testing via
  @puppeteer/browsers; no unzip needed, ~150 MB) > system Chrome/Chromium/Edge/Brave > Playwright's cached Chromium.
  `dev-browser install-skill [--claude|--codex|--agents]` writes SKILL.md to ~/.claude|.codex|.agents/skills/dev-browser (default all).

## migration
  dev-browser 1.x uses ~/.dev-browser/v1 so it never connects to the incompatible 0.2.x daemon or reads its profiles.
  To carry state forward from the predecessor implementation, first run `doobie stop`, then:
    dev-browser migrate-from-doobie
  This copies config, browser profiles, named-page maps, downloaded Chrome, saved files, and launch preferences from
  ~/.doobie (or DOOBIE_HOME) into DEV_BROWSER_HOME. It never modifies the source and refuses to overwrite existing v1
  state. The old dev-browser 0.2.x Playwright profiles are intentionally left in ~/.dev-browser and are not compatible.
...
## json
  ...
  The process exit code equals done.exitCode. Output is never capped in --json mode.  jq: dev-browser --json -e '...' |
  jq -c 'select(.type=="result").data'

## mcp
  dev-browser mcp [-b NAME] [--headless] [--connect URL] [-t S]    Model Context Protocol server over stdio (JSON-RPC 2.0).
  Tools: dev_browser_run { script, browser?, headless?, connect?, timeout? } (console output, return value, errors, page.shot()
  images as image content), dev_browser_pages, dev_browser_browsers, dev_browser_stop { browser? }, dev_browser_help { topic? }. Flags given
  to `dev-browser mcp` are defaults for every call; same daemon and named pages as the CLI, so CLI scripts and MCP calls share
  pages. Claude Code: `claude mcp add dev-browser -- dev-browser mcp --headless`.
...
## examples
  ...
  dev-browser --headless -e 'const p = await browser.getPage("hn"); await p.goto("https://news.ycombinator.com");
  ...
  dev-browser -e 'const p = await browser.getPage("shop"); await p.shot({ name: "shop.jpg" })'
  dev-browser -e 'const p = await browser.getPage("shop"); await p.mouse.click(412, 233); await p.waitForLoad(); p.url()'
  # Your own Chrome (after `dev-browser chrome`): list tabs, attach one by target id, scoped snapshot with boxes
  dev-browser --connect -e 'await browser.listPages()'
  dev-browser --connect -e 'const p = await browser.getPage("4F0C...32HEX"); await p.snapshot({ scope: "e42", boxes: true })'
  # Test a local dev app: console errors surface automatically on stderr as [page:dev] lines
  dev-browser -t 20 <<'EOF'
  ...
  dev-browser --ignore-https-errors -e 'const p = await browser.getPage("dev"); await p.goto("https://localhost:8443"); p.url()'
  dev-browser -e 'const p = await browser.getPage("shop"); ({ url: p.url(), title: await p.title(), shot: (await p.shot()).path })'
  dev-browser -e 'const p = await browser.getPage("embed"); await p.click("ref/f1e3"); (await p.ref("f1e3")).evaluate(e => e.textContent)'
...
## tips
  ...
  - --headless for unattended work; headed (default) to watch (separate profiles). Claude Code: allowlist `Bash(dev-browser *)`.
  - Long jobs: --idle-timeout 0 keeps the browser alive; `dev-browser stop` when done. -b NAME isolates logins/parallel work.
    For Google/OAuth logins use `dev-browser chrome` + `--connect`; automation-launched Chrome is often blocked.
