# Driving a host-owned Chromium (bb in-app browser) with doobie

doobie can attach to any Chromium that a host application owns, without a
devtools port, through the **socket source**:

```
doobie --connect unix:/path/to/cdp.sock -e '...'
```
...
## Behavior on the doobie side

- The browser key is `socket:unix:/path`; named pages live under that key in
  `~/.doobie/pages/`.
- Nothing is launched or killed; `doobie stop socket:unix:/path` only
  disconnects.
