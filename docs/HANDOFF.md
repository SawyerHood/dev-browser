...
4. **Known, documented limits** (candidates for code fixes): relative file paths in `uploadFile`/`screenshot({path})`/`pdf({path})` resolve against the daemon's cwd (could resolve via `RunRequest.cwd`); `readFile` cannot read `~/.doobie/tmp/downloads/<name>` (add `readDownload(name)`?); `waitForSelector("ref/eN")` timeout is a plain `Error`, not `TimeoutError`; ref on an element moved to another frame acts on wrong coordinates; sync CPU loops are not preempted by the deadline (watchdog restart); a long single action (type with delay) holds the front lock for other tabs.
