...
- `page.snapshot({ scope?, interactive?, depth?, track?, boxes?, urls?, maxChars?, frames? }) -> string | { full, incremental }`. Full ARIA YAML (ported in-page script, isolated world), refs on every visible pointer-receiving element (headings included; `interactive` prunes), and same-origin/cross-origin iframe traversal with frame-prefixed refs `f1e5`. Frame keys are stable for one frame document (including same-document navigation); reload/full navigation/renderer swaps get fresh, never-reused keys. Snapshot evaluation is bound to its captured execution context, and routed queries retain the full prefix for in-page document validation, closing navigation races before side effects. Unavailable or detached-during-snapshot frames are marked `[unavailable]`. The default cap is ~20k chars with a truncation marker that names the narrowing options; `track` gives line diffs with one context line per hunk (first call = full; a >60 % change falls back to the full snapshot), `boxes` adds `[x,y,w,h]` in main-viewport px, `urls: false` drops `/url` lines.
