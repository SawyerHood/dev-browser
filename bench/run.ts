/**
 * dev-browser benchmark suite.
 *
 *   bun run bench/run.ts [--check] [--bin PATH|dev] [--runs N]
 *
 * Runs against an isolated DEV_BROWSER_HOME and a warm headless daemon, prints a
...
/** DEV_BROWSER_BENCH_SCALE multiplies every target (CI runners are slower than a dev box). */
const SCALE = Math.max(1, Number(process.env.DEV_BROWSER_BENCH_SCALE ?? "1") || 1);
...
async function main(): Promise<void> {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "dev-browser-bench-"));
  const env: Record<string, string> = { ...(process.env as Record<string, string>), DEV_BROWSER_HOME: home };
  delete env.NODE_PATH;

  const distBin = path.join(ROOT, "dist/dev-browser");
  ...
  const runDevBrowser = async (args: string[], stdin?: string): Promise<Res> => {
  ...
  const script = async (code: string): Promise<{ ms: number; value: string | undefined }> => {
    const r = await runDevBrowser(["--headless", "--json", "--quiet-page", "-e", code]);
  ...
  try {
    ...
    await measure(
      ...
      async () => {
        ...
        const b = await runDevBrowser(["--json", "browsers"]);
  ...
  console.log("");
  if (SCALE !== 1) console.log(`(targets scaled x${SCALE} via DEV_BROWSER_BENCH_SCALE)`);
