...
`DOOBIE_HOME` temp dir and a warm headless daemon. Everything is cleaned up at
...
- Uses `dist/doobie` when it exists (`bun run build`), else `bun run src/cli/main.ts`
...
| --- | --- | --- |
| `1+1` | wall time of one `doobie --headless -e '1+1'` process | 25 ms |
