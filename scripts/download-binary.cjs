// Shared by scripts/postinstall.cjs and bin/doobie.cjs: locate, download and
// verify the prebuilt doobie binary for this platform, and (for global npm
// installs) point the global `doobie` symlink straight at it.
//
// Env knobs:
//   DOOBIE_SKIP_DOWNLOAD=1       never download (postinstall and shim both honour it)
//   DOOBIE_DOWNLOAD_BASE=URL     full URL prefix holding doobie-<os>-<arch> + SHA256SUMS
//                                (default https://github.com/<REPO>/releases/download/v<VERSION>)
//   DOOBIE_REPO=owner/name       GitHub repo used to build the default base
//   DOOBIE_VERSION=x.y.z         release version used to build the default base
"use strict";
...
const binPath = path.join(pkgRoot, "bin", process.platform === "win32" ? "doobie-bin.exe" : "doobie-bin");
/** The JS shim (what npm links `doobie` to before relinkGlobal runs). */
const shimPath = path.join(pkgRoot, "bin", "doobie.cjs");
...
function downloadBase() {
  const b = process.env.DOOBIE_DOWNLOAD_BASE;
...
function platformAsset() {
  ...
  return { asset: `doobie-${plat}-${arch}`, plat, arch };
}
...
async function downloadBinary(opts = {}) {
  ...
  const { asset } = p;
  log(`doobie: downloading ${asset} v${VERSION} from ${base}`);
  ...
  log(`doobie: installed binary at ${dest}`);
  return { dest, asset, base };
}

/**
 * For a *global* npm install only: if `<prefix>/bin/doobie` is a symlink that
 * resolves to THIS package's bin/doobie.cjs, re-point it at the binary so Node
...
function relinkGlobal(opts = {}) {
  ...
  try {
    ...
    if (opts.log) opts.log(`doobie: ${link} now points directly at the binary`);
    return true;
  } catch {
...
function manualHint() {
  const p = platformAsset();
  const asset = p.asset || "doobie-<os>-<arch>";
  const lines = [
    `  - run \`npm rebuild -g doobie\` (re-runs the download; make sure DOOBIE_SKIP_DOWNLOAD is unset),`,
